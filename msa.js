const { store, hiveClient, processor } = require('./index')
    ...
    return new Promise((resolve, reject) => {
        store.get(['msa'], (err, result) => {
            if (err || Object.keys(result).length === 0) {
                ...
                now = Date.parse(processor.getBlockHeader(`${num % 1000}`).timestamp + '.000Z'),
                op = {
                    ref_block_num: processor.getBlockHeader(`${num % 1000}`).block_number & 0xffff,
                    ref_block_prefix: Buffer.from(processor.getBlockHeader(`${num % 1000}`).block_id, 'hex').readUInt32LE(4),
